...
module.exports = function(thorin, opt, unloqApi) {
  ...
  dispatcher
    ...
    .use((intentObj, next) => {
      ...
      unloqApi.tokenData(authToken, apiOpt).then((userData) => {
        ...
        // prepare the userData to include other details from UNLOQ.
        delete userData.source_client;
        let AccountStoreModel = storeObj.model(opt.modelName, true),
          qry = {},
          whereQry = [];
        let tmpQry = {};
        tmpQry[opt.unloqId.field] = userData.id
        whereQry.push(tmpQry);
        if(AccountStoreModel.fields['email']) {
          whereQry.push({
        ...
        AccountModel.find(qry).then((accObj) => {
          ...
          if (accObj) {
            ...
            onAccountLogin(accObj, userData, (err) => {
              if(err) return next(thorin.error(err));
              intentObj
                .data(opt.modelName, accObj)
                .data('unloq', userData);
          ...
          accObj.set('email', userData.email);
          if (opt.loginAt) {
          ...
          logger.error(`Failed to query for account ${userData.id} on uauth login`);
          logger.debug(err);
          next(thorin.error('AUTH.UNLOQ.ERROR', 'An error occurred. Please try again', 500, err));
        });
      }).catch((err) => {
   ...
   * */
  function onAccountLogin(accountObj, userData, done) {
    if (!opt.loginAt) return;
    accountObj.set(opt.loginAt.field, Date.now());
    accountObj.set(opt.unloqId.field, userData.id);
    accountObj.set('email', userData.email);
